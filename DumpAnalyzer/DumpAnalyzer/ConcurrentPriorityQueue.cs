...
namespace DebugDiag.DumpAnalyzer
{
    ...
    {
        private readonly Func<TValue, TKey> _keyExctractionFunction;
        ...
        public ConcurrentPriorityQueue(Func<TValue,TKey> keyExctractionFunction)
        {
            _keyExctractionFunction = keyExctractionFunction;
        }
        ...
        public ConcurrentPriorityQueue(IEnumerable<KeyValuePair<TKey, TValue>> collection,Func<TValue, TKey> keyExctractionFunction):this(keyExctractionFunction)
        {
        ...
        {
            Enqueue(new KeyValuePair<TKey, TValue>(_keyExctractionFunction(value),value));
        }
        ...
        {
            lock (_syncLock) _minHeap.Items.Select(x=>x.Value).ToList().CopyTo(array, index);
        }
